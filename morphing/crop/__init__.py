...
# Default color for drawn rectangles (color selector was removed)
RECT_COLOR = 0x00AA00
...
def CropCutDialog(Action):
    ...
    """
    # Get selected series (used for default area info and to pre-check target)
    ...
    try:
        ...
        # Target Section
        # =====================================================================
        sep_tgt = vcl.TBevel(Form)
        sep_tgt.Parent = Form
        sep_tgt.Left = 10
        sep_tgt.Top = 210
        sep_tgt.Width = 470
        sep_tgt.Height = 2
        sep_tgt.Shape = "bsTopLine"
        ...
        # Helper to create/update a single target series with its filtered points
        def output_series(tgt, new_points, operation_name):
            """Create a new series (same color as tgt) or replace tgt in place."""
            if rb_new.Checked:
                ...
                # Same color as the original series
                ...
                if tgt.LegendText:
                    tgt.LegendText = f"{tgt.LegendText} [{operation_name}]"

        # Generic runner: applies a predicate to every checked target series
        def run_operation(predicate, operation_name):
            targets = [s for chk, s in series_checks if chk.Checked]
            if not targets:
                show_error("Select at least one target series.", "Crop/Cut")
                return
            ...
            empty = []
            for tgt in targets:
                tx, ty = get_series_data(tgt)
                if not tx:
                    continue
                new_points = [Point(x, y) for x, y in zip(tx, ty)
                              if predicate(x, y, xleft, xright, ybottom, ytop)]
                if not new_points:
                    empty.append(tgt.LegendText if tgt.LegendText else "Series")
                    continue
                output_series(tgt, new_points, operation_name)
                total_points += len(new_points)
                processed += 1

            Graph.Update()

            msg = f"{operation_name} completed on {processed} series. {total_points} points total."
            if empty:
                msg += f"\n\nSkipped (no points remaining): {', '.join(empty)}"
            show_info(msg, "Crop/Cut")

        # Predicates --------------------------------------------------------
        def on_crop_x(Sender):
            run_operation(lambda x, y, xl, xr, yb, yt: xl <= x <= xr, "Crop X")

        def on_crop_y(Sender):
            run_operation(lambda x, y, xl, xr, yb, yt: yb <= y <= yt, "Crop Y")

        def on_cut_x(Sender):
            run_operation(lambda x, y, xl, xr, yb, yt: x < xl or x > xr, "Cut X")

        def on_cut_y(Sender):
            run_operation(lambda x, y, xl, xr, yb, yt: y < yb or y > yt, "Cut Y")

        def on_crop_xy(Sender):
            run_operation(lambda x, y, xl, xr, yb, yt: xl <= x <= xr and yb <= y <= yt, "Crop XY")

        def on_cut_xy(Sender):
            run_operation(lambda x, y, xl, xr, yb, yt: not (xl <= x <= xr and yb <= y <= yt), "Cut XY")
        ...
        def on_draw_rect(Sender):
            ...
            # Use default rectangle color
            rect_series.LineColor = RECT_COLOR
            rect_series.FillColor = RECT_COLOR
            rect_series.FrameColor = RECT_COLOR
